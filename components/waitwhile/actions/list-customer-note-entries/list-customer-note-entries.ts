...
export default defineAction({
  ...
  description:
    "List customer note entries. [See the doc here](https://developers.waitwhile.com/reference/getcustomerscustomeridnotes)",
  props: {
    waitwhile,
    customerId: {
  ...
  async run({ $ }) {
    try {
      const data = await this.waitwhile.listCustomerNoteEntries(
        this.customerId,
      );
