...
export default defineAction({
  ...
  description:
    "Remove a user invite. [See the doc here](https://developers.waitwhile.com/reference/deleteinvitesinviteid)",
  props: {
    waitwhile,
    inviteId: {
      propDefinition: [waitwhile, "inviteId"],
    },
  },
  type: "action",
  methods: {},
  async run({ $ }) {
    try {
      ...
      throw new Error(
        `Error status code: ${statusCode}. Error status response: ${statusText}. You need a Paid Plan to use this API `,
      );
    }
  },
});
