  ...
  propDefinitions: {
    ...
    userKey: {
      ...
      async options({ prevContext }) {
        ...
        const { users, nextPageToken } = await this.listUsers({
          pageToken,
        });
        const options = users?.map(({ name: label, primaryEmail: value }) => ({
          label,
          value,
        }));
        return {
  ...
  methods: {
    ...
    async listUsers(args = {}) {
      try {
        ...
        const { data } = await users.users.list(args);
        return data;
      } catch (error) {
