...
import requests
from code_gen.generate_for_github_issue import (generate_from_issue,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir",
        "--output_dir",
        help="The path for the output dir",
        default=os.path.join("..", "..", "components"),
    )
    parser.add_argument(
        "--num-tries",
        "--num_tries",
        dest="tries",
        help="The number of times we call the model to generate code",
        type=int,
        default=2,
    )
    parser.add_argument(
    ...
    if args.issue:
        ...
        instructions = requests.get(
            f"https://api.github.com/repos/PipedreamHQ/pipedream/issues/{args.issue}"
        ).json()["body"]
